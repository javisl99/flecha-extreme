...
export default function ReservasPage() {
  ...
  const initialLoadingCompletedRef = useRef(false);
  ...
  useEffect(() => {
    if (initialLoadingCompletedRef.current) {
      return;
    }
    ...
    if (loading) {
      if (initialLoadingStartedAtRef.current === null) {
    ...
    if (initialLoadingStartedAtRef.current === null) {
      ...
      initialLoadingCompletedRef.current = true;
      return;
    }
  ...
  const renderCalendarSkeleton = () => (
    <section className="rounded-[1.5rem] border border-outline-variant/30 bg-surface-container-lowest p-4 shadow-card-ambient sm:p-6">
      <div className="rounded-[1.25rem] border border-outline-variant/20 bg-surface-container-low p-4 sm:p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="space-y-2">
            <LoadingSkeletonBlock className="h-7 w-40" />
            <LoadingSkeletonBlock className="h-4 w-28" />
          </div>
          <div className="flex items-center gap-2">
            {Array.from({ length: 3 }).map((_, index) => (
              <LoadingSkeletonPill key={index} className="h-9 w-9" />
            ))}
          </div>
        </div>
        ...
        <div className="mt-5 overflow-x-auto">
          <div className="grid min-w-[48rem] grid-cols-7 gap-3">
            {Array.from({ length: 7 }).map((_, index) => {
              const showSecondary = index % 2 === 0;

              return (
                <div key={index} className="overflow-hidden rounded-xl border border-outline-variant/20 bg-surface-container-low">
                  <div className="flex items-center justify-between gap-2 border-b border-outline-variant/15 bg-surface-container-lowest px-3 py-2">
                    <div className="space-y-1">
                      <LoadingSkeletonBlock className="h-3 w-8 rounded-full" />
                      <LoadingSkeletonBlock className="h-4 w-12 rounded-full" />
                    </div>
                    <LoadingSkeletonPill className="h-6 w-6" />
                  </div>

                  <div className="space-y-2 p-2.5">
                    <div className="rounded-lg border border-primary/10 bg-primary/6 p-2">
                      <LoadingSkeletonBlock className="h-3 w-16 rounded-full" />
                      <LoadingSkeletonBlock className="mt-2 h-3 w-4/5 rounded-full" />
                    </div>

                    <div className="rounded-lg border border-outline-variant/15 bg-surface-container-lowest p-2">
                      <LoadingSkeletonBlock className="h-3 w-14 rounded-full" />
                      <LoadingSkeletonBlock className="mt-2 h-2.5 w-3/5 rounded-full" />
                    </div>

                    {showSecondary ? (
                      <LoadingSkeletonPill className="h-5 w-20" />
                    ) : (
                      <LoadingSkeletonBlock className="h-2.5 w-16 rounded-full" />
                    )}
                  </div>
                </div>
              );
  ...
  const renderInitialLoadingSkeleton = () => (
    <div className="page-container space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="space-y-2">
          <LoadingSkeletonBlock className="h-9 w-52" />
          <LoadingSkeletonBlock className="h-4 w-36" />
        </div>
        <div className="flex items-center gap-3">
          <LoadingSkeletonBlock className="h-12 w-12 rounded-full" />
  ...
  if (showInitialLoadingScreen) {
    return renderInitialLoadingSkeleton();
  }
