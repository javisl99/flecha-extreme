...
?>
<div id="control_container_<?php echo intval( $random ); ?>" class="grid_layout_container">
	<?php
	if ( isset( $gallery_image_detail_only_included_images ) ) {
		if ( count( $gallery_image_detail_only_included_images ) > 0 ) {
			foreach ( $gallery_image_detail_only_included_images as $pic ) {
				...
				?>
				<div id="grid_wrapper_item_<?php echo intval( $random ) . '_' . intval( $pic['id'] ); ?>" class="grid_wrapper_item gm_animate"  data-animate="<?php echo isset( $animation_effects ) ? esc_attr( $animation_effects ) : 'none'; ?>" data-duration="1.0s" data-delay="0.1s" data-offset="100">
				<?php
				...
				?>
				<div id="grid_item_image_<?php echo intval( $random ) . '_' . intval( $pic['id'] ); ?>" class="grid_item_image_<?php echo intval( $random ); ?> <?php echo 'video' === $pic['file_type'] ? 'grid_item_video_' . intval( $random ) : ''; ?>" style="background:url('<?php echo 'image' === $pic['file_type'] ? esc_attr( $filename_thumbs ) : esc_attr( $pic['video_thumb'] ); ?>');background-size:100% 100% !important;">
					<img src="<?php echo 'image' === $pic['file_type'] ? esc_attr( $filename_thumbs ) : esc_attr( $pic['image_name'] ); ?>" image_full_path="<?php echo esc_attr( $pic['file_type'] ); ?>" alt="<?php echo esc_attr( $pic['alt_text'] ); ?>" id="ux_gm_file" name= "ux_gm_file_<?php echo intval( $random ) . '_' . intval( $pic['id'] ); ?>"/>
				</div>
			</a>
				<?php
				if ( '' !== $pic['image_title'] || '' !== $pic['image_description'] ) {
				...
				<div id="grid_content_item" class="grid_content_item">
					...
					if ( 'show' === $thumbnail_title && '' !== $pic['image_title'] ) {
						?>
						<div id="grid_single_text_title_<?php echo intval( $random ) . '_' . intval( $pic['id'] ); ?>" class="grid_single_text_title">
					...
					if ( 'show' === $thumbnail_description && '' !== $pic['image_description'] ) {
						?>
						<div id="grid_single_text_desc_<?php echo intval( $random ); ?>" class="grid_single_text_desc">
